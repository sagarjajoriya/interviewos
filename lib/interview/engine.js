...
function buildTurnMessages(config, history) {
  ...
  if (last?.role === "user") {
    // Merge the note into the candidate's message rather than adding a second
    // consecutive user turn — models anchor their reply on the final user
    // message, and a standalone note can eclipse the actual answer.
...
export async function generateReport(config, history) {
  ...
  // Models occasionally emit malformed/truncated JSON even in JSON mode;
  // one retry resolves the vast majority of transient failures.
  let lastErr;
